...
function ItemPage() {
      ...
      const filterData = () => {
        ...
        // 2 필터
          ...
          // 1+ 2 둘다 필터
            ...
            x = [
              ...content.filter(data => data.hashtag_id === hashtag_id && data.hashtag_id === hashtag_id),
            ];
            console.log(x.length);
          }
    ...
    return(
      ...
      <Modal />
      <div>
        <button id="all" onMouseDown={handleFilterButton}>전체</button>
        <button id="1" onMouseDown={handleFilterButton}>효도템</button>
        <button id="2" onMouseDown={handleFilterButton}>생신</button>
      ...
      <div>
        {filteredList.map((data, idx) => {
         ...
         {content.map(i => <UserInputContainer key={i.post_id}>{i.content}</UserInputContainer>)}
      </Container>
    )
}
